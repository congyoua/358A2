...
void A_send(struct msg message) {
    ...
    if (seq_s > base+N) return;

    packet[seq_s].seqnum = seq_s;

    strncpy(packet[seq_s].payload, message.data, MSG_SIZE);
    for (i=0; i<strlen(packet[seq_s].payload); i++){
        cs += (int)packet[seq_s].payload[i];
    }
    packet[seq_s].checksum = ~cs;
    copy[seq_s] = packet[seq_s];
    to_network_layer(0, packet[seq_s]);

    if (base==seq_s) starttimer(0, 10);
    seq_s++;
}

/* called from network layer, when a packet arrives for transport layer */
void A_recv(struct pkt packet) {
    if (packet.checksum!=copy.checksum) return;
...
void A_timeout(void) {
    int i;
    starttimer(0, 10);
    for (i=base;i<seq_s-1;i++){
        to_network_layer(0, copy[i]);
    }
}
